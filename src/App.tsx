...
export default function App() {
  ...
  // FIX 1: isScrolling phải là useRef thay vì biến let thông thường
  // để giữ giá trị qua các lần re-render và tránh scroll loop
  const isScrollingRef = useRef(false);

  // FIX 2: Dùng useCallback để tránh tạo lại hàm mỗi lần render
  const handleSidebarScroll = useCallback(() => {
    if (isScrollingRef.current || !sidebarRef.current || !timelineRef.current) return;
  ...
  // FIX 3: useEffect sync scroll đặt SAU khi visibleTasks được khai báo
  // (trước đây đặt trước visibleTasks → lỗi "used before declaration")
  useEffect(() => {
  ...
  const ganttData = useMemo(() => {
    ...
    if (ganttMode === 'month') {
      while (current <= maxDate) {
        ...
        const isLastOfGroup = current.getMonth() === 11;
        periods.push({
  ...
  const ganttHeaderGroups = useMemo(() => {
    ...
    periods.forEach((p) => {
      const label = ganttMode === 'month'
        ? `Năm ${p.date.getFullYear()}`
        : `Tháng ${p.date.getMonth() + 1}/${p.date.getFullYear()}`;
  ...
  // FIX 4: scrollToTask đặt SAU khi taskPositions được khai báo
  // (trước đây đặt trước taskPositions → stale closure, không thấy dữ liệu mới nhất)
          ...
          if (fromPos && toPos) {
            ...
            arrows.push(
              <path
                ...
                // FIX 6: Bỏ markerStart, chỉ giữ markerEnd để mũi tên chỉ đúng hướng
                markerEnd="url(#dot)"
                              ...
                              <td className="px-8 py-5">
                                <div className="flex items-center gap-2" style={{ paddingLeft: depth * 24 }}>
                                  ...
                                  <div>
                                    ...
                                    {/* FIX 5: Hiển thị predecessors đúng dạng "3, 5SS, 7FF"
                                        thay vì "[object Object]" do gọi .join() trên mảng object */}
                                    <div className="text-xs text-slate-400 mt-0.5">
                                      ID: #{task.id}{task.predecessors && ` • Pre: ${task.predecessors.map(p => `${p.id}${p.type !== 'FS' ? p.type : ''}`).join(', ')}`}
                                    </div>
                                  </div>
                                </div>
                              </td>
